...
class InboundPlanningController extends Controller
{
    ...
    private function getSupplier($supplier_id = false)
    {
        $data = DB::query()
            ...
            ->leftJoin("m_warehouse as d", "d.client_project_id", "=", "b.client_project_id")
    ...
    private function getSKU($sku = null)
    {
    ...
    //     ])
    //     ->from("m_wh_item as a")
    //     ->leftJoin("m_warehouse c","c.wh_id","=","a.wh_id")
    //     ->leftJoin("m_wh_client_project as b",function ($query)
    //     {
    //         $query->on("b.client_id","=","a.client_id");
    //         $query->on("c.client_project_id","=","b.client_project_id");
    ...
    public function show(Request $request, $id)
    {
        ...
        $current_data_detail = DB::select("SELECT 
            ...
            LEFT JOIN m_warehouse AS c ON c.client_project_id = b.client_project_id AND c.wh_id = g.wh_id
    ...
    private function getTargetUserAssign($username = false)
    {
        $data = DB::query()
            ...
            ->leftJoin("m_warehouse as e", "e.client_project_id", "=", "d.client_project_id")
            ->where("e.wh_id", session('current_warehouse_id'))
            ->where("d.client_id", session('current_client_id'))
    ...
    public function getFinishInboundDataForMobileApp($checker, Request $request)
    {
        // Fetch data from the database using Query Builder
        $dt = DB::table('t_wh_inbound_planning as ip')
            ->join('t_wh_activity as a', 'ip.inbound_planning_no', '=', 'a.inbound_planning_no')
            ->select('a.checker', 'ip.inbound_planning_no', 'ip.status_id', 'a.location_from', 'a.datetime_created')
            ->where('ip.status_id', 'FIN')
            ->where('a.checker', $checker)
            // ->whereDate('a.datetime_created', now()->format('Y-m-d'))
            ->orderBy('a.datetime_created', 'desc')
            ->get();
    ...
    public function getInboundPlanningDetailForMobileApp($inboundPlanningNo, $checker)
    {
        // Fetch data from the database using Query Builder with join
        $dt = DB::table('t_wh_inbound_planning_detail as d')
            ->join('t_wh_inbound_planning as p', 'd.inbound_planning_no', '=', 'p.inbound_planning_no')
            ->join('t_wh_activity as a', 'd.inbound_planning_no', '=', 'a.inbound_planning_no')
            ->select('d.*', 'p.status_id', 'a.activity_id', 'a.checker')
            ->where('d.inbound_planning_no', $inboundPlanningNo)
            ->where('a.checker', $checker)
            ->get();
    ...
    public function processSavePartialVehicleMobileApp(Request $request, $inboundPlanningNo, $checker)
    {
        // Mendapatkan data inbound planning
        $check_Inbound_Planning = $this->getInboundPlanningDetailForMobileApp($inboundPlanningNo, $checker);

        // Cek apakah data ditemukan
        if ($check_Inbound_Planning->isNotEmpty()) {
            // Ambil status_id dan activity_id
            $status_id = $check_Inbound_Planning[0]->status_id;
            $activity_id = $check_Inbound_Planning[0]->activity_id;

            // Jika status_id sama dengan 'UIN', lakukan insert atau update
            if ($status_id === 'UIN') {
                // Validasi input
                try {
                    $request->validate([
                        ...
                        'start_unloading' => 'required|date',
                        'user_created' => 'required|string',
                        'is_active' => 'required|in:Y,N',
                        'is_deleted' => 'required|in:Y,N'
                    ]);
                ...
                $existingEntry = DB::table('t_wh_transportation')
                    ->where('activity_id', $activity_id)
                ...
                if ($existingEntry) {
                    ...
                    DB::table('t_wh_transportation')
                        ->where('transport_id', $existingEntry->transport_id)
                        ->update([
                    ...
                    $id = DB::table('t_wh_transportation')->insertGetId([
                        'activity_id' => $activity_id,
                ...
                // Jika status_id tidak 'UIN', mengembalikan response dengan pesan
                return response()->json(['message' => 'Status ID tidak valid untuk insert'], 400);
            }
        } else {
            return response()->json(['message' => 'Checker salah atau status_id bukan UIN untuk inbound planning no: ' . $inboundPlanningNo], 400);
        }
    }
    ...
    public function getDetailWHTransportation($activity_id, $vehicle_no)
    {
        $dt = DB::query()
            ->from("t_wh_transportation")
            ->where('activity_id', $activity_id)
    ...
    public function processSaveFinishVehicleMobileApp(Request $request, $activity_id, $vehicle_no)
    {
        $check_detail_wh_transport = $this->getDetailWHTransportation($activity_id, $vehicle_no);

        // Cek apakah data ditemukan
        if ($check_detail_wh_transport->isEmpty()) {
            return response()->json(['message' => 'Parameter activity_id salah atau tidak ada data vehicle_no: ' . $vehicle_no], 404);
        }

        // Ambil detail pertama
        $detail = $check_detail_wh_transport[0];

        try {
            // Validasi input
            $request->validate([
                'finish_unloading' => 'required|date',
                'departure_date' => 'required|date',
                'user_updated' => 'required|string',
            ]);
        } catch (ValidationException $e) {
            return response()->json(['errors' => $e->validator->errors()], 422);
        }

        // Cek apakah arrival_date dan start_unloading terisi
        if (empty($detail->arrival_date) || empty($detail->start_unloading)) {
            return response()->json(['message' => 'arrival_date dan start_unloading harus terisi'], 400);
        }
        ...
        try {
            DB::table('t_wh_transportation')
                ...
                ->where('vehicle_no', $vehicle_no)
                ->update([
                    'finish_unloading' => $request->input('finish_unloading'),
                    'departure_date' => $request->input('departure_date'),
                    'user_updated' => $request->input('user_updated'),
                    'datetime_updated' => now(),
                ]);
    ...
    public function getVehicleForMobileApp()
    {
        $dt = DB::query()
            ->select([
                "a.vehicle_id",
    ...
    // MOBILE APP - PROSES SCAN
    public function getStockTypeScanMobileApp($stock_id = false)
    {
        $data = DB::query()
            ->select([
                "a.stock_id",
    ...
    public function getSKUforMobileApp($inbound_planning_no)
    {
        ...
        $data = DB::table('t_wh_inbound_planning_detail')
    ...
    public function checkQtyPlan(Request $request)
    {
        ...
        $check_qty_plan = DB::select("SELECT 
          * FROM (
              SELECT 
                  ...
                  b.uom_name
              FROM 
                  t_wh_inbound_planning a
              LEFT JOIN 
                  t_wh_inbound_planning_detail AS b ON b.inbound_planning_no = a.inbound_planning_no
              LEFT JOIN 
                  t_wh_activity c ON a.inbound_planning_no = c.inbound_planning_no AND a.reference_no = c.reference_no AND b.inbound_planning_no = c.inbound_planning_no
              LEFT JOIN 
        ...
        // Cek hasil query
        if (count($check_qty_plan) == 0) {
            return response()->json([
                "err" => true,
                "message" => "For this SKU, check qty plan does not exist",
                "data" => [],
            ], 200);
        }
    ...
    public function getInboundPlanningDetails($inboundPlanningNo, $sku)
    {
        $details = DB::table('t_wh_inbound_planning AS a')
            ->leftJoin('t_wh_inbound_planning_detail AS b', 'b.inbound_planning_no', '=', 'a.inbound_planning_no')
            ->select('b.SKU', 'b.serial_no', 'b.item_name', 'b.uom_name', 'b.batch_no', 'b.stock_id')
    ...
    public function checkOutstanding($inboundPlanningNo, $sku)
    {
        // Melakukan query untuk mendapatkan data outstanding
        $result = DB::table('t_wh_inbound_planning as a')
            ->join('t_wh_inbound_planning_detail as b', 'b.inbound_planning_no', '=', 'a.inbound_planning_no')
            ->leftJoin('t_wh_activity as c', function ($join) {
                $join->on('a.inbound_planning_no', '=', 'c.inbound_planning_no')
                    ...
                    ->on('b.inbound_planning_no', '=', 'c.inbound_planning_no');
            })
            ...
            ->select(
                ...
                'b.expired_date',
                'b.serial_no',
                DB::raw('b.qty AS qty_plan'),
                DB::raw('COALESCE(SUM(d.qty_scan), 0) AS qty_scan'),
                DB::raw('b.qty - COALESCE(SUM(d.qty_scan), 0) AS qty_outstanding'),
                'b.uom_name'
            )
            ->where('a.inbound_planning_no', $inboundPlanningNo)
            ->where('b.sku', $sku)
    ...
    public function isOutStandingZero($inboundPlanningNo)
    {
        // Melakukan query untuk mendapatkan data outstanding
        $result = DB::table('t_wh_inbound_planning as a')
            ->join('t_wh_inbound_planning_detail as b', 'b.inbound_planning_no', '=', 'a.inbound_planning_no')
            ...
            ->select(
                DB::raw('CASE WHEN (b.qty - COALESCE(SUM(d.qty_scan), 0)) = 0 THEN "true" ELSE "false" END AS is_outstanding_zero')
            )
            ->where('a.inbound_planning_no', $inboundPlanningNo)
            ->groupBy('b.sku', 'b.item_name', 'b.batch_no', 'b.expired_date', 'b.serial_no', 'b.qty', 'b.uom_name')
            ->first();
    ...
    public function processScanMobileApp(Request $request, $inboundPlanningNo, $activityId, $checker)
    {
        $check_Inbound_Planning = $this->getInboundPlanningDetailForMobileApp($inboundPlanningNo, $checker);

        if ($check_Inbound_Planning->isEmpty()) {
            return response()->json(['message' => 'Checker salah atau tidak ditemukan untuk inbound planning no: ' . $inboundPlanningNo], 400);
        }
        ...
        if ($status_id === 'UIN') {
            ...
            try {
                $request->validate([
                    ...
                    'details.*.qty_plan' => 'required|integer',
                    'details.*.pallets' => 'required|array',
                    'details.*.pallets.*.pallet_id' => 'required|string',
                    'details.*.pallets.*.qty_scan' => 'required|integer',
                ]);
            } catch (ValidationException $e) {
                return response()->json(['errors' => $e->validator->errors()], 422);
            }
            ...
            foreach ($request->details as $detail) {
                // Memeriksa detail SKU dalam inbound planning
                $check_SKU_in_inbound_planning_detail = $this->getInboundPlanningDetails($inboundPlanningNo, $detail['sku']);

                if ($check_SKU_in_inbound_planning_detail->isEmpty()) {
                    return response()->json(['message' => 'Tidak ada SKU dalam Inbound Planning: ' . $inboundPlanningNo], 400);
                }

                // Cek qty_outstanding
                $outstandingData = $this->checkOutstanding($inboundPlanningNo, $detail['sku']);
                $QtyPlan = $outstandingData->qty_plan;

                // Ambil qty_scan yang sudah ada di outstandingData
                $existingQtyScan = $outstandingData->qty_scan ?: 0;

                // Hitung total qty_scan untuk pallets baru
                $totalQtyScan = $existingQtyScan;

                foreach ($detail['pallets'] as $pallet) {
                    $totalQtyScan += (int)$pallet['qty_scan'];
                }

                // Cek apakah total qty_scan melebihi qty_plan
                if ($totalQtyScan > $QtyPlan) {
                    return response()->json([
                        'status' => 'failed',
                        'message' => 'Total qty_scan tidak boleh melebihi qty_plan untuk SKU: ' . $detail['sku']
                    ], 400);
                }

                // Jika semua pengecekan lolos, lakukan insert
                foreach ($detail['pallets'] as $pallet) {
                    $id = DB::table('t_wh_scan_qty')->insertGetId([
                        'activity_id' => $activityId,
                        'transport_id' => $request->transport_id,
                        'pallet_id' => $pallet['pallet_id'],
                        'sku' => $detail['sku'],
                        'stock_id' => $request->stock_id,
                        'qty_scan' => (int)$pallet['qty_scan'],
                        "part_name" => @$check_SKU_in_inbound_planning_detail[0]->item_name,
                        "batch_no" => @$check_SKU_in_inbound_planning_detail[0]->batch_no,
                        "uom_name" => @$check_SKU_in_inbound_planning_detail[0]->uom_name,
                        'is_active' => "Y",
            ...
            // Mengembalikan response
            return response()->json(['status' => 'success', 'message' => 'Data berhasil diproses'], 201);
        } else {
    ...
    {
        // Mendapatkan data inbound planning
        $check_Inbound_Planning = DB::table('t_wh_inbound_planning_detail as d')
            ->join('t_wh_inbound_planning as p', 'd.inbound_planning_no', '=', 'p.inbound_planning_no')
            ->join('t_wh_activity as a', 'd.inbound_planning_no', '=', 'a.inbound_planning_no')
            ->select('d.*', 'p.status_id', 'a.activity_id', 'a.checker')
            ->where('d.inbound_planning_no', $inboundPlanningNo)
        ...
        // Cek apakah data ditemukan
        if ($check_Inbound_Planning) {
            // Ambil status_id dan activity_id
            $status_id = $check_Inbound_Planning->status_id;
            $activity_id = $check_Inbound_Planning->activity_id;

            // Jika status_id sama dengan 'UIN', ambil data transportasi
            if ($status_id === 'UIN') {
                $transportationData = DB::table('t_wh_transportation as t')
                    ->join('t_wh_activity as a', 't.activity_id', '=', 'a.activity_id')
                    ->join('t_wh_inbound_planning as p', 'a.inbound_planning_no', '=', 'p.inbound_planning_no')
                    ->select('t.*')
                    ->where('p.inbound_planning_no', $inboundPlanningNo)
                ...
                return response()->json([
                    'inboundPlanning' => $check_Inbound_Planning,
                    'transportation' => $transportationData
                ], 200);
            } else {
                return response()->json(['message' => 'Status ID tidak valid untuk mendapatkan data transportasi'], 400);
            }
        } else {
            return response()->json(['message' => 'Checker salah atau status_id bukan UIN untuk inbound planning no: ' . $inboundPlanningNo], 400);
        }
