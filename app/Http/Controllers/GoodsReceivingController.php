...
class GoodsReceivingController extends Controller
{
    ...
    private function getDataGoodsReceiving($gr_id = false)
    {
        $data = DB::query()
            ->select([
                ...
                "g.client_name",
                DB::raw("CASE 
                        WHEN COUNT(tmd.movement_id) > 0 
                                AND SUM(CASE WHEN tmd.is_scanned = 'Y' THEN 1 ELSE 0 END) = COUNT(tmd.movement_id) 
                            THEN 'fully_scan' 
                            ELSE 'not_fully_scan' 
                        END AS scan_status")
            ])
            ...
            ->leftJoin("t_wh_receive_detail as rd", "rd.gr_id", "=", "a.gr_id")
            ->leftJoin("t_wh_temporary_movement as tmd", "tmd.movement_id", "=", "rd.movement_id")
    ...
    private function getGoodReceiveDetails($gr_id = false)
    {
        $data = DB::table("t_wh_inbound_planning_detail as a")
            ->select([
                "tm.movement_id",
                "a.sku",
                "a.item_name",
            ...
            ->leftJoin("t_wh_inbound_planning as d", "d.inbound_planning_no", "=", "a.inbound_planning_no")
            ->leftJoin("t_wh_receive as e", "e.inbound_planning_no", "=", "a.inbound_planning_no")
            ->leftJoin("t_wh_receive_detail as rd", "rd.gr_id", "=", "e.gr_id")
            ->leftJoin("t_wh_temporary_movement as tm", function ($join) {
                $join->on("tm.movement_id", "=", "rd.movement_id")
                    ->on("tm.sku", "=", "a.sku");
            })
            ->when($gr_id, function ($query) use ($gr_id) {
    ...
    public function processReceiveDetail(Request $request, $id)
    {
        ...
        if (!in_array($current_data->status_id, ["RGR"])) {
            return response()->json([
                "err" => true,
    ...
    // PUT AWAY MOBILE APP
    public function getOrderPutAway($warehouseman)
    {
        $dataOrderPutAway = DB::table('t_wh_temporary_movement as tm')
            ->join('t_wh_receive_detail as rd', 'tm.movement_id', '=', 'rd.movement_id')
            ->join('t_wh_receive as r', 'rd.gr_id', '=', 'r.gr_id')
            ->select(
                'rd.gr_id',
            ...
            ->where('tm.warehouseman', $warehouseman)
            ->where('r.status_id', 'RGR')
            ->get();

        return response()->json($dataOrderPutAway, 200);
    }
    ...
    public function updateScanStatus(Request $request)
    {
        try {
            ...
            // Kembalikan response dengan error validasi
        ...
        $warehouseman = $request->input('warehouseman');
        $isScanned = $request->input('is_scanned') ? 'Y' : 'N';
        $inputQty = $request->input('qty');

        // Cek apakah ada data yang sesuai di tabel t_wh_temporary_movement
        $record = DB::table('t_wh_temporary_movement')
            ->where('movement_id', $movementId)
            ->where('sku', $sku)
            ->where('location_to', $location_to)
            ->where('warehouseman', $warehouseman)
            ->first();

        if ($record) {
            // Cek apakah qty yang diinput sesuai
            if ($record->qty != $inputQty) {
                return response()->json(['status' => 400, 'message' => 'Quantity you entered does not match.'], 400);
            }

            // Update status pemindaian di tabel t_wh_temporary_movement
            $updated = DB::table('t_wh_temporary_movement')
                ->where('movement_id', $movementId)
                ->where('sku', $sku)
                ->where('warehouseman', $warehouseman)
                ->where('location_to', $location_to)
                ->update(['is_scanned' => $isScanned]);

            // Cek apakah ada data yang diupdate
            if ($updated) {
                return response()->json(['status' => 200, 'message' => 'Update status scan successful.'], 200);
            } else {
                return response()->json([
                    'status' => 202,
                    'message' => "This data with movement ID '$movementId' and SKU '$sku' does not need to be scanned again."
                ], 202);
            }
        } else {
    ...
    public function getPutawayData($warehouseman)
    {
        $putaway_planning = DB::table('t_wh_temporary_movement as tm')
            ...
            ->where(function ($query) {
                $query->where('tm.is_scanned', '!=', 'Y')
                    ->orWhereNull('tm.is_scanned')
                    ->orWhere('tm.is_scanned', '');
            })
            ->where('r.status_id', 'RGR')
            ->get();


        $finishScan = DB::table('t_wh_temporary_movement as tm')
            ->join('t_wh_receive_detail as rd', 'tm.movement_id', '=', 'rd.movement_id')
            ->join('t_wh_receive as r', 'rd.gr_id', '=', 'r.gr_id')
            ->select('rd.gr_id', 'tm.movement_id', 'tm.sku', 'tm.qty', 'tm.location_to', 'tm.warehouseman', 'tm.is_scanned')
            ->where('warehouseman', $warehouseman)
            ->where('is_scanned', 'Y')
            ->where('r.status_id', 'CGR')
            ->get();


        $result = [
            'putaway_planning' => $putaway_planning,
            'putaway_finished' => $finishScan,
        ];
